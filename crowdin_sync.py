...
# directly to DarkNess-reDefined Github.
#
...
def push_as_commit(base_path, path, name, branch, username):
    ...
    try:
        repo.git.commit(m='DarkNess-reDefined Automatic translation import')
    except:
    ...
    # Push commit
    try:
        repo.git.push('git@github.com:DarkNess-reDefined/%s' % (name),
                      'HEAD:%s' % branch)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Synchronising DarkNess-reDefined translations with Crowdin")
    ...
    sync.add_argument('--no-upload', action='store_true',
                      help='Only download DarkNess-reDefined translations from Crowdin')
    sync.add_argument('--no-download', action='store_true',
                      help='Only upload DarkNess-reDefined source translations to Crowdin')
